...
cp -r ~/dev/fixkit/eval /vol/tmp/werkkai/eval_fitness_$SLURM_ARRAY_TASK_ID
cd /vol/tmp/werkkai/eval_fitness_$SLURM_ARRAY_TASK_ID

# Run the Python script with the job index as an argument
python ./eval_student_assignments_copy.py $SLURM_ARRAY_TASK_ID

#cleanup
rm -r /vol/tmp/werkkai/eval_fitness_$SLURM_ARRAY_TASK_ID